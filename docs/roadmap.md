...
- [x] 反馈生成 eval draft。
- [x] 30 条固定黄金集和四项 CI 阈值。
- [x] pytest、Vitest、Playwright、Docker Compose 健康检查。
- [x] 上传/URL 安全、限流、脱敏日志与可选 Sentry。

## 下一版本：0.2 Durable Local

目标：让单用户长期使用时的数据、索引和升级更可靠。

- [ ] 正式支持本地 sentence-transformer embedding profile。
- [ ] Chroma 持久化的迁移、备份和维度检查测试。
- [ ] chunker/embedding/index version 写入 metadata。
- [ ] 后台索引任务的本地队列实现，避免大文件阻塞请求。
- [ ] PDF 版面、表格和 OCR 质量分级。
- [ ] 导出/导入知识库清单和评测集。
- [ ] 扩充到 100+ 困难 case，加入同名文档、长文档和冲突证据。
- [ ] 延迟、拒答率和引用覆盖分布 dashboard。

退出条件：重启、备份恢复、模型迁移和失败重试都有自动化证据。
