...
| 文件 | 类型 | 用途 | 来源 |
| --- | --- | --- | --- |
| `multimodal-rag-hero.png` | 1806×871 PNG | README 宽幅主视觉 | Codex 内置图像生成 |
| `system-overview.svg` | SVG | 输入、入库、检索、回答和改进全景 | 仓库内代码原生绘制 |
| `retrieval-pipeline.svg` | 1440×720 SVG | 十阶段多模态与图谱检索追踪 | 仓库内代码原生绘制 |
| `evaluation-loop.svg` | SVG | 反馈到 CI 的质量闭环 | 仓库内代码原生绘制 |
| `deployment-modes.svg` | SVG | 三种部署成熟度 | 仓库内代码原生绘制 |
| `request-lifecycle.svg` | 1440×660 SVG | 浏览器到响应的请求生命周期 | 仓库内代码原生绘制 |
| `data-model.svg` | 1440×780 SVG | 当前 SQLite/向量数据与生产迁移边界 | 仓库内代码原生绘制 |
...
| `evaluation-scorecard.svg` | 1440×900 SVG | 100 条基础/多模态/图谱十二项成绩卡 | 仓库内代码原生绘制 |
| `social-preview.svg` | 1280×640 SVG | GitHub 社交预览图的确定性源文件 | 仓库内代码原生绘制 |
| `social-preview.png` | 1280×640 PNG | GitHub 设置上传与英文 README 预览 | 从同名 SVG 确定性导出 |

## 主视觉生成说明

主视觉是仓库唯一的生成式概念插画，使用内置 image generation 生成，核心 prompt：

> 用于开源多模态 RAG 证据工作台的宽幅 GitHub README 主视觉：PDF、网页、图片和笔记依次经过词法检索、向量检索、排序、证据门和引用检查，形成有据可查的回答卡片。深海军蓝、青绿、蓝绿和紫色；克制高级的 3D/矢量风格；无人像、标识、可读文字或水印。
