...
## 2026-07-23 至 2026-07-29 实际执行快照
...
| 模型提供方 | Ollama 0.32.1 原生/兼容嵌入通过且为 768 维；三类 `qwen3:8b` 生成请求均 180 秒超时；DeepSeek 官方 `GET /models` 连接验证通过，生成质量未计作已验收 |
| Sentry | 阻断：无 DSN，未发送真实事件；Docker 7.8 GiB，不满足 14 GiB 完整配置内存门 |
| 人工标注 / 真实问题 | 0/200、0/100；200 条机器候选只计 `draft` |
| 14 天观测 | 真实宿主机/运行时间隔均已入链并自然重置；截至 `2026-07-28T17:47:03Z` 共 1,170 个样本、27 个失败样本，最长连续 81,984 秒。当前窗口从 `2026-07-28T16:26:46Z` 开始，已记录 4,817 秒，尚未达标且不能回填 |
...
本次只读复核同时确认 Production Compose 9/9 服务健康，Redis Streams `pending=0`、`lag=0`，索引任务成功 200 个；2 条 DLQ 为保留的历史审计记录。宿主卷使用率 85%、可用 67 GiB，后续观测需继续关注磁盘增长。
