...
生成模型即使“知道”答案，也只有在最终召回的叶级证据通过门禁后才会被调用。对带版本、型号或其他直接标识符的问题，最终证据必须覆盖问题中的全部精确标识符和相邻实体组合。只召回 `v1` / `v3` 不能推断 `v2`，`Borealis v2` 也不能支撑 `Aurora v2`。这类情况固定拒答，并且在同步回答和流式回答中都不进入生成器。

### 4.2 轻量检索健康信号

`retrieval_trace.pipeline.retrieval_health` 记录 BM25 / Dense Top-10 重合、通道进入最终证据的比例、重复率、文档集中度、跨问题 Top-K Jaccard 和“万能块”信号。在线比较只针对同一索引版本、检索范围和路由，且只比较 token Jaccard 不高于 0.20 的不相关问题；相同问题的重试不重复入窗。

窗口最多保留 128 条规范化 token 集和叶块 ID，只存在当前进程内，重启即清空；不写新数据库或日志文件，不引入模型或第三方依赖。被阻断、已降级、`summary` 或少于 3 个候选的请求不进入跨问题窗口。在线小样本信号只用于 trace、Prometheus 和系统健康页预警，不会自动改排序、放行答案或触发回滚；发布仍以锁定回归集和真实盲测为准。
